...
public class VídeoeManager : MonoBehaviour
{
    public VideoPlayer Patoinicio; // Assign your VideoPlayer component in the Inspector
    ...
    private void Start()
    {
        ...
        Patoinicio.Prepare(); // Prepare the video for playback
        Patoinicio.loopPointReached += OnVideoEnd; // Subscribe to the loopPointReached event
    }
    ...
    private IEnumerator LoadDemoSceneAfterDelay()
    {
        ...
        SceneManager.LoadScene("menu"); // Replace "Demo" with the actual scene name
    }

    private void OnDestroy()
    {
        Patoinicio.loopPointReached -= OnVideoEnd; // Unsubscribe from the event on destruction
    }
}